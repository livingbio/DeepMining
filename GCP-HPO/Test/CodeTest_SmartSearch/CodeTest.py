...
def test2():
	...
	search = SmartSearch(parameters,model='GP',estimator=scoring_function,n_iter=15,n_init=10,n_final_iter=3)
	search._fit()
...
def gp_vs_random_search(test_name,n_tests,search_lenght,save_data=False):
	...
	print 'GP_ucb search'
	for i in range(n_tests):
		ucb_search = SmartSearch(parameters,estimator=pipeline,X=X,y=y,
	...
	all_random_results = []
	for i in range(n_tests):
		random_search = SmartSearch(parameters,estimator=pipeline,X=X,y=y,
